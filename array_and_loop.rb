...
# 4.9 さまざまな繰り返し処理
# 4.9.1 times メソッド
# 配列を使わず n 回処理を繰り返したい、という場合はIntegerクラスのtimesメソッドを使う
...
# 4.9.3 step メソッド
# 1、3、5、7のように、nからmまで数値をx個ずつ増やしながら何か処理をしたい場合は、Numericクラスのstepメソッドを使う
# stepメソッドの仕様
...
# しかし、1行で書くのであれば修飾子としてwhile文を後ろに置いたほうがスッキリ書ける
a = []
a << 1 while a.size < 5
p a #=>[1,1,1,1,1]

# どんな条件でも最低1回は実行したい、という場合は begin...end で囲んでからwhileを書く
a = []
while false
...
# while文やuntil文を書く場合は無限ループを発生させないように注意が必要
